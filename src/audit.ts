...
export async function auditSkill(path: string, options: { config?: Partial<Config>; provider?: LlmProvider; semanticReview?: unknown } = {}): Promise<Report> {
  ...
  return { schemaVersion: '1.2', toolVersion: '0.3.1', scoringPolicy: 'design-review-v1', createdAt: new Date().toISOString(), skill: { name: skill.name, fingerprint: skill.fingerprint, files: skill.files.map(({ path, hash }) => ({ path, hash })) }, config, metrics, findings, patches, refactors: planRefactors(patches), checks: discoverChecks(skill), scores: scoreDimensions(findings, semantic.status === 'completed', assessments), semantic };
}
