...
适用于使用当前 Agent 宿主读取技能原文并完成语义审查的完整体检。

用户明确要求完整、八维或语义审查时，直接进入本流程；明确要求快速、静态或离线时不进入本流程。用户只说检测、体检或审查时，先说明快速静态审查与完整八维审查的范围和耗时，等待选择。选择完整八维审查后，已经配置并启用的远程模型服务，或用户此前已明确选择的服务，视为已有数据处理授权，可直接使用并在报告中记录 provider。只有新增或切换到没有既有配置/授权记录的远程地址时，才在发送前说明目标与发送范围；授权后不逐次提醒。
...
完整报告要求 `semantic.status=completed`，且八个维度均为 `assessed` 并有数值分数。完整模式中的静态预扫描是中间产物，不单独交付为评分结论。普通 `audit` 未指定模式时会保存静态暂评并退出 3，以阻止宿主把它误当作完整体检；用户选择快速静态审查时使用 `audit --static`。
